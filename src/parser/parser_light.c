...
/*   Updated: 2023/09/25 14:12:47 by aguilmea         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	parse_brightness(char *file_string, int *index, t_element *element)
{
	...
	if (onecolor < 0 || onecolor > 1.0)
		return (false);
	element->light.intensity = color(onecolor, onecolor, onecolor);
	return (true);
}
...
static bool	modify_brightness_with_color(char *file_string, int *index, t_element *element)
{
	...
	if (parse_color(file_string, index, &modif) == false)
		return (false);
	element->light.intensity.red *= modif.red;
	element->light.intensity.green *= modif.green;
	element->light.intensity.blue *= modif.blue;
	return (true);
}
