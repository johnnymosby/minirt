...
/*   Updated: 2023/10/18 13:29:25 by aguilmea         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	print_infos(t_win *win)
{
	...
	m = win->mlx_ptr;
	p = win->win_ptr;
	mlx_set_font(m, p, "-sony-fixed-medium-r-normal--24-170-100-100-c-120-jisx0201.1976-0");
	mlx_string_put(m, p, PCT_WIDTH + 5, 50, WHITE, "Control Keys");
	mlx_set_font(m, p, "-misc-fixed-medium-r-normal--0-0-75-75-c-0-iso10646-1");
...
void	render_menu(t_win *window)
{
	...
	mlx_put_image_to_window(window->mlx_ptr, window->win_ptr, window->menu.img_ptr, PCT_WIDTH, 0);
	print_infos(window);
}